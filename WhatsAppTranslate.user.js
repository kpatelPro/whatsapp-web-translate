...
function main() {
	...
	var checkSelectableText = function(obj) {
		var summary = obj[0];
		summary.added.forEach(function(newEl) {
		    var selectableText = $(newEl);
			var html = selectableText.html();
			if ((typeof html != 'undefined') && (html !== null)) {
				//alert('checkselectabletext html: ' + html);
...
function extractResultDom(responseText)
{
if (limit == 1)
	console.log(responseText);
limit = 0;

	var html = responseText.match(/\<body[^\>]*\>([\s\S]+)\<\/body\>/)[1];
