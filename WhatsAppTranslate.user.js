...
function main() {
	...
	var msgCheck = function(obj) {
		...
		summary.added.forEach(function(msg) {
			var message = $(msg).children(".message.message-in").first();
			if ((typeof message != 'undefined') && (message !== null)) {
				var bubbleText = $(message).find(".bubble.bubble-text").first();
			    var messageText = $(bubbleText).children(".message-text").first();
			    var messageEmojiText = $(messageText).children(".emojitext").first();
			    manipulateMessageElement(messageEmojiText, message, "left");
			}
		});
	};
	...
	var lastMsgCheck = function(obj) {
		...
		summary.added.forEach(function(msg) {
			...
			if ((typeof lastMsg != 'undefined') && (lastMsg !== null)) {
			    var messageEmojiText = $(lastMsg).children(".emojitext").first();
			    manipulateMessageElement(messageEmojiText, lastMsg, "right");
			}
		});
	};
...
function manipulateMessageElement(messageEmojiText, uiParent, cssFloatButton)
{
    var html = messageEmojiText.html();
	if ((typeof html == 'undefined') || (html === null)) {
		return;
	}
	//alert('checkBubble html: ' + html);
	...
	translateButton.addEventListener ("click", function() {
		try {
			...
			handleTranslationRequest(strs, $(translationSpan), $(translationUI));
		} catch (e) {
			console.log(e);
		}
	});
...
function handleTranslationRequest(strs, span, ui)
{
	var srcText = strs[2];
